...
def get_sdk_info_via_aapt(apk_path):
    ...
    try:
        ...
        return None, None, "aapt 未找到"
    except Exception as e:
        return None, None, str(e)

def parse_manifest_attributes(unpack_dir):
    """从反编译后的 AndroidManifest.xml 解析属性"""
...
def scan_smali_for_permission_usage(unpack_dir, dangerous_perms):
    ...
    if not os.path.exists(smali_dir):
        return {}
    perm_patterns = {perm: re.compile(re.escape(perm)) for perm in dangerous_perms}
    result = {perm: {'checked': 0, 'unchecked': []} for perm in dangerous_perms}
        ...
        for file in files:
            ...
            for perm in dangerous_perms:
                if perm_patterns[perm].search(content):
                    if 'checkSelfPermission' in content:
...
def generate_deep_report(apk_path, unpack_dir):
    """
    生成深度分析报告
    apk_path: APK 的路径字符串（已确保是 str 类型）
    unpack_dir: 反编译目录路径（字符串或 None）
    """
    # 关键修正：splitext 返回 (name, ext)，只取 [0] 得到字符串[2](@ref)
    ...
    if unpack_dir and os.path.exists(os.path.join(unpack_dir, 'smali')):
        ...
        report_lines.append("  [跳过] 需要反编译目录（smali 目录不存在）")
    report_lines.append("")
    ...
    report_lines.append("")
    report_lines.append("(分析基于 aapt dump badging 方法 及 Smali 静态扫描)")
    return '\n'.join(report_lines)
...
def main():
    ...
    apk_path = sys.argv[1]          # 正确获取第一个参数（字符串）[3](@ref)
