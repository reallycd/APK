...
def get_aapt_path():
    ...
    使用 os.path.abspath 和 os.path.dirname 确保路径准确性[1](@ref)[3](@ref)
    """
    # 获取当前脚本的绝对路径所在目录
    script_path = os.path.abspath(sys.argv)  # sys.argv 是脚本路径（字符串）
    script_dir = os.path.dirname(script_path)   # 脚本所在目录
    candidates = [
        os.path.join(script_dir, 'aapt.exe'),   # 与脚本同一目录
        os.path.join(script_dir, 'aapt'),        # 同目录下 Linux 可执行文件
        'aapt.exe',                              # 当前工作目录
        'aapt'                                   # 系统 PATH
    ]
    for candidate in candidates:
        if os.path.exists(candidate):
            return candidate
    # 如果都不存在，返回 'aapt' 让后续调用触发更清晰的 FileNotFoundError
    return 'aapt'

def get_sdk_info_via_aapt(apk_path):
    """
    使用 aapt dump badging 获取真实的 minSdkVersion 和 targetSdkVersion
    参考 Android 官方文档：aapt 命令可解析 APK 的 manifest 和 resources[4](@ref)
    """
    aapt = get_aapt_path()
    try:
        result = subprocess.run(
            ...
            check=False  # 不抛出 CalledProcessError，由我们自己处理返回值
        )
...
def parse_manifest_attributes(unpack_dir):
    ...
    try:
        ...
        if uses_sdk is not None:
            attrs['minSdkVersion'] = uses_sdk.get(ns['android'] + 'minSdkVersion', '')
            attrs['targetSdkVersion'] = uses_sdk.get(ns['android'] + 'targetSdkVersion', '')
            attrs['maxSdkVersion'] = uses_sdk.get(ns['android'] + 'maxSdkVersion', '')
        # 检查 package 和 version
        attrs['package'] = root.get('package', '')
        attrs['versionCode'] = root.get(ns['android'] + 'versionCode', '')
        attrs['versionName'] = root.get(ns['android'] + 'versionName', '')
        return attrs
...
def check_resources_arsc(apk_path):
    ...
    某些 APK 将 targetSdkVersion 编译进 arsc 文件，仅修改 Manifest 无效[4](@ref)
    """
    aapt = get_aapt_path()
    try:
        result = subprocess.run(
...
def scan_smali_for_permission_usage(unpack_dir, dangerous_perms):
    ...
    for perm in dangerous_perms:
        # 在 smali 中权限字符串通常以 "android.permission.XXX" 形式出现
...
def generate_deep_report(apk_path, unpack_dir):
    ...
    生成深度分析报告
    apk_path: APK 的绝对路径
    unpack_dir: 反编译目录（可为 None 则只进行 aapt 检查）
    """
    ...
    if unpack_dir and os.path.exists(os.path.join(unpack_dir, 'smali')):
        ...
        report_lines.append("  [跳过] 需要反编译目录（smali 目录不存在），请先运行 ApkTool 解包")
    report_lines.append("")
    ...
    if unpack_dir and os.path.exists(os.path.join(unpack_dir, 'smali')):
        ...
        report_lines.append("  [跳过] 需要反编译目录")
    report_lines.append("")
    ...
    if target_sdk:
        try:
            ...
            report_lines.append("  targetSdkVersion 格式异常，请人工检查。")
    else:
        report_lines.append("  targetSdkVersion 未知，建议升级至 31+ 并全面适配。")

    report_lines.append("")
    report_lines.append("(分析基于 aapt dump badging 方法[4](@ref) 及 Smali 静态扫描)")
    return '\n'.join(report_lines)
...
def main():
    ...
    apk_path = sys.argv[1]          #  ✅ 正确：取第一个实际参数
    ...
    if unpack_dir and not os.path.exists(unpack_dir):
        print(f"警告: 反编译目录 {unpack_dir} 不存在，将跳过 Smali 扫描部分")
        unpack_dir = None
